...
function isBooleanSequence(node: t.Node): boolean {
  ...
  return (
    (t.isUnaryExpression(node) || t.isBinaryExpression(node)) && isBoolean(node)
  );
}
...
function getTail(path: NodePath<t.Statement>) {
  ...
  if (index === -1) {
    throw new Error(
      "Unable to find the current node among it's parent's childs.",
    );
  }
