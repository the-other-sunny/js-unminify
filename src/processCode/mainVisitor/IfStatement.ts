...
import { isExpandable, willReturn, isInLambda, getTail, negate } from '../../astUtils';
...
function ANDSequence1(
    path: NodePath<IfStatement<LogicalANDExpression, Consequent, null | undefined>>
) {
    // if (c && seq)
    //     cons;
    // ===>
...
function ORSequence2(
    path: NodePath<IfStatement<LogicalORExpression, Consequent, null | undefined>>
) {
    // function() {
    //     ...
    //     if (c || seq)
...
export default function(
    ...
    if (t.isSequenceExpression(test)) {
        sequenceExpressionTest(
            path as NodePath<IfStatement<t.SequenceExpression, Consequent, Alternate>>
        );
        return;
    }
    ...
    if (
        ...
        ANDSequence1(
            path as NodePath<IfStatement<LogicalANDExpression, Consequent, null | undefined>>
        );
        return;
    }
    ...
    if (
        ...
        ANDSequence2(
            path as NodePath<IfStatement<LogicalANDExpression, Consequent, t.Statement>>
        );
        return;
    }
    ...
    if (
        ...
        ORSequence1(
            path as NodePath<IfStatement<LogicalORExpression, Consequent, t.Statement>>
        );
        return;
    }
    ...
    if (
        ...
        if (tail.length === 0) {
            ORSequence2(
                path as NodePath<IfStatement<LogicalORExpression, Consequent, null | undefined>>
            );
            return;
        }
