...
interface ReturnStatement<T extends t.ReturnStatement["argument"]>
  extends t.ReturnStatement {
  argument: T;
}

type ExtractableAssignment = AssignmentExpression<
...
function returnSequence(path: NodePath<ReturnStatement<t.SequenceExpression>>) {
...
function returnVoid(path: NodePath<ReturnStatement<VoidExpression>>) {
  if (!isExpandable(path)) {
    console.warn(
      `Given \`path\` is not expandable.\n` +
        `    Node type: ${path.node.type}\n` +
...
function returnUndefined(path: NodePath<ReturnStatement<UndefinedIdentifier>>) {
...
function returnLogicalOR(path: NodePath<ReturnStatement<LogicalORExpression>>) {
  const { left, right } = path.node.argument;
...
export default function (path: NodePath<t.ReturnStatement>) {
  ...
  if (t.isSequenceExpression(argument)) {
    returnSequence(path as NodePath<ReturnStatement<t.SequenceExpression>>);
    return;
  }
  ...
  if (t.isUnaryExpression(argument) && argument.operator === "void") {
    returnVoid(path as NodePath<ReturnStatement<VoidExpression>>);
    return;
  }
  ...
  if (
    ...
    returnAssignment(path as NodePath<ReturnStatement<ExtractableAssignment>>);
    return;
  }

  if (t.isIdentifier(argument) && argument.name === "undefined") {
    returnUndefined(path as NodePath<ReturnStatement<UndefinedIdentifier>>);
    return;
  }
  ...
  if (t.isLogicalExpression(argument) && isBoolean(argument.left)) {
    ...
    if (argument.operator === "&&")
      returnLogicalAND(path as NodePath<ReturnStatement<LogicalANDExpression>>);
    if (argument.operator === "||")
      returnLogicalOR(path as NodePath<ReturnStatement<LogicalORExpression>>);
    return;
  }
}
