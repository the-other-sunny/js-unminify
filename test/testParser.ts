...
function testParser(inputPath: string) {
  ...
  function extractTestBlock(testBlock: t.LabeledStatement) {
    ...
    const [inputBlock, expectedOutputBlock] = (
      testBlock.body as t.BlockStatement
    ).body as t.LabeledStatement[];

    const inputAst = t.file(
      t.program((inputBlock.body as t.BlockStatement).body),
    );
    const expectedOutputAst = t.file(
      t.program((expectedOutputBlock.body as t.BlockStatement).body),
    );
